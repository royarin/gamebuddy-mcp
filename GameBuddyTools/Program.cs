...
var apiBaseUrl = configuration["GameBuddyApi:BaseUrl"] ?? "https://localhost:5001";
...
var agent = chatClient.AsAIAgent(
    ...
    name: agentName,
    tools: [
        AIFunctionFactory.Create(GetAllKidsNames),
        AIFunctionFactory.Create(GetKidProfile)
        ]
);
...
[Description("Gets a kid's profile (name and age) by their nickname")]
async Task<string> GetKidProfile([Description("The nickname of the kid")] string nickname)
{
    // call API to get profile info for the kid
    // call GameBuddy.API to get profile info for the kid
    try
        {
            var response = await httpClient.GetAsync($"/kids/{nickname}/profile");
            
            if (!response.IsSuccessStatusCode)
            {
                return $"Kid with nickname '{nickname}' not found.";
            }
        ...
        {
            return $"Error retrieving profile: {ex.Message}";
        }
}

async Task<string> GetAllKidsNames()
{
    try
    {
        var response = await httpClient.GetAsync("/kids");
        
        if (!response.IsSuccessStatusCode)
        {
            return "Error retrieving kids list.";
        }
        ...
        return $"Kids: {string.Join(", ", kids)}";
    }
    catch (Exception ex)
    {
        return $"Error retrieving kids: {ex.Message}";
    }
}